...
%Presecne tacke za dijagram Phi(q)


%Izolovana zona od 0 do 0,2q, jer tu nije relevantno blokiranje tockova
%==========================================================================
% %Optereceno
% 
% 
% qopres=q_o(q_o>0.2);
 qvel1=size(q_o);
% qvel2=size(qopres);
% n=(qvel1(1,2)-qvel2(1,2));
% 
 inc_o=qvel1(1,2);



%Izbacivanje n prvih clanova iz niza
yp=phi_po;
yz=phi_zo;
% 
% if n==0
...
%Funkcija za odredjivanje krive po jednaakoj podeli
ptp = interparc(inc_o,q_o,yp,'spline');
ptp=ptp';
x1=ptp(1,:);
y1=ptp(2,:);

ptz = interparc(inc_o,q_o,yz,'spline');
ptz=ptz';
x2=ptz(1,:);
y2=ptz(2,:);
...
%Provera gde se kriva prednje osovine na athezionom dijagramu sece sa
%linijom q=phi
[qoppr_phi,phi_pogrpr]=curveintersect(x1,y1,qo_dij,phi_oq);

%--------------------------------------------------------------------------

%Provera gde se kriva zadnje osovine na athezionom dijagramu sece sa
%linijom q=phi
[qozpr_phi,phi_zogr]=curveintersect(x2,y2,qo_dij,phi_oq);  
%--------------------------------------------------------------------------    
  

%Neoptereceno

% qnopres=q_no(q_no>0.2);
qnvel1=size(q_no);
% qnvel2=size(qnopres);
% m=(qnvel1(1,2)-qnvel2(1,2));
% inc_no=qnvel2(1,2)-m;
...
% else
%     for bpno=1:1:m
% bno1(1,bpno)=ypno(1,bpno);
% bno2(1,bpno)=yzno(1,bpno);
%     end
%     ypno(ismember(ypno,bno1))=[];
%     yzno(ismember(yzno,bno2))=[];
% end
% 


%Funkcija za odredjivanje krive po jednaakoj podeli
ptp = interparc(inc_no,q_no,ypno,'spline');
ptp=ptp';
x11=ptp(1,:);%q
y11=ptp(2,:);%phi

ptz = interparc(inc_no,q_no,yzno,'spline');
ptz=ptz';
x21=ptz(1,:);%q
y21=ptz(2,:);%phi
...
[qnoppr_phi,phi_pnogrprno]=curveintersect(x11,y11,qno_dij,phi_noq);  
...
%Presecna tacka
    [qoppr_phi1,phi_pogrpr1]=curveintersect(x1,y1,qo1_dij,phi_oq1);

%Neoptereceno
     %Kriva fi=(q1+0.07)./0.85.
    qno1_dij_inc=1.2./dimno;
    qno1_dij=qno1_dij_inc:qno1_dij_inc:1.2;
    phi_noq1=(qno1_dij+0.07)./0.85;

%Presecna tacka
[qnoppr_phi1,phi_pnogrpr1]=curveintersect(x21,y21,qno1_dij,phi_noq1);
...
%--------------------------------------------------------------------------    
    ...
    if isempty(qprus1o)
       %Presecna tacka krivih u opterecenom i neopterecenom stanju 
             %Optereceno
            [qoppr_phi,phi_preso]=curveintersect(x1,y1,x2,y2); 
   end
    
    if isempty(qprus1no)
    %Neoptereceno
            [qnoppr_phi,phi_presno]=curveintersect(x11,y11,x21,y21);
...
qpr_bool=getappdata(0,'qpr_bool');
  ...
  warndlg('Vozilo zadovoljava uslove sa stanovista kocnih performansi prema pravilniku ','Informacija!!')
            
   else
                
  warndlg('Vozilo ne zadovoljava uslove sa stanovista kocnih performansi prema pravilniku','Upozorenje!!')
...
%nece dostici vrednost od 1g na toj osovini...     
    ...
    if qnoppr>qnozpr
     ...
     warndlg('Prvo ce blokirati zadnji tockovi vozila a to ce prouzrokovati nestabilnost vozila','Upozorenje!!')  
